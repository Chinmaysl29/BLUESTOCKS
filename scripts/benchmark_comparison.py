...
Executes your exact code line by line:
    ...
    nifty50 = benchmark[benchmark["index_name"].str.contains("NIFTY 50", case=False, na=False)]
...
print("Line 1  nifty50 = benchmark[benchmark['index_name'].str.contains('NIFTY 50', ...)]")
...
# NOTE: benchmark uses "NIFTY50" (no space) — str.contains catches it
# if match count is 0, fall back to "NIFTY50" exact
if len(nifty50) == 0:
    print("  ⚠️  No match for 'NIFTY 50' — falling back to 'NIFTY50'")
    nifty50 = benchmark[benchmark["index_name"] == "NIFTY50"]
    print(f"  Fallback shape: {nifty50.shape}")
    print()

# Line 2
plt.figure(figsize=(12, 6))
print("Line 2  plt.figure(figsize=(12, 6))  ✅")

# Line 3 — plot Top 5 avg NAV
plt.plot(top_nav["date"], top_nav["nav"], label="Top 5 Funds",
         color="steelblue", linewidth=2)
print("Line 3  plt.plot(top_nav['date'], top_nav['nav'], label='Top 5 Funds')  ✅")

# Line 4 — plot NIFTY 50
plt.plot(nifty50["date"], nifty50["close_value"], label="NIFTY 50",
         color="darkorange", linewidth=1.8, linestyle="--")
print("Line 4  plt.plot(nifty50['date'], nifty50['close_value'], label='NIFTY 50')  ✅")

# Line 5
plt.legend()
print("Line 5  plt.legend()  ✅")

# Line 6
plt.title("Top Funds vs NIFTY 50")
print("Line 6  plt.title('Top Funds vs NIFTY 50')  ✅")
...
plt.close()
print(f"Line 8  plt.savefig('{out}')  ✅")
print()
